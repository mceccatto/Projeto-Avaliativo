...
public class Bacharelado extends Avaliacao {
	
	private final double Media = 6.0;
	private double Nota;
	
	public Bacharelado(Long Curso, Long Materia, Long Aluno, double Nota, String DataRegistro, Connection Conexao) {
		super(Curso, Materia, Aluno, DataRegistro, Conexao);
		this.Nota = Nota;
	}
	
	public Bacharelado(Long Curso, Long Aluno, Connection Conexao) {
		super(Curso, Aluno, Conexao);
	}
	
	public Bacharelado(String Tipo, Long Curso, Long Materia, Long Aluno, double Nota) {
	...
	public String validar() {
		if(this.Conexao != null) {
			...
			try {
				...
				if(statusGeral.equalsIgnoreCase("APROVADO(A)") || statusGeral.equalsIgnoreCase("APROVADO(A) COM PENDÊNCIAS")) {
