...
public class Mestrado extends Avaliacao {
	...
	public String validar() {
		if(this.Conexao != null) {
			...
			PreparedStatement stm = null;
			try {
				stm = Conexao.prepareStatement("SELECT A.Tipo AS Tipo, A.Curso AS CursoId, C.Nome AS CursoNome, A.Materia AS MateriaId, D.Nome AS MateriaNome, A.Aluno AS AlunoId, B.Nome AS AlunoNime, A.Conceito AS Conceito, A.Status AS STATUS FROM avaliacoes A JOIN alunos B ON B.Id = A.Aluno JOIN cursos C ON C.Id = A.Curso JOIN materias D ON D.Id = A.Materia WHERE A.Curso = ? AND A.Aluno = ?");
